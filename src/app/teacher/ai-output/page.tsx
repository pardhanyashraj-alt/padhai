...
function AIOutputContent() {
  ...
  console.log("AIOutputContent Params:", { type_raw, book, chapter_label, subject, grade, classId, initialMode, backend_type });
  ...
  const [pptStatus, setPptStatus] = useState<"not_generated" | "generating" | "ready" | "failed">("not_generated");
  const [pptUrl, setPptUrl] = useState<string | null>(null);
  const [pptPolling, setPptPolling] = useState(false);
  ...
  const fetchPPTStatus = useCallback(async () => {
    if (!bookId) return;
    try {
      const res = await apiFetch(`/teacher/books/${bookId}/ppt`);
      if (res.ok) {
        const data = await res.json();
        console.log("PPT Backend Response:", data);
        setPptStatus(data.ppt_status);
        setPptUrl(data.ppt_url || null);
        if (data.ppt_status === "generating") {
          setPptPolling(true);
        } else {
          setPptPolling(false);
        }
      } else {
        console.error("PPT Fetch Failed, Status:", res.status);
      }

    } catch (err) {
      console.error("Failed to fetch PPT status:", err);
    }
  }, [bookId]);

  useEffect(() => {
    let interval: NodeJS.Timeout;
    if (pptPolling && bookId) {
      interval = setInterval(fetchPPTStatus, 3000);
    }
    return () => {
      if (interval) clearInterval(interval);
    };
  }, [pptPolling, bookId, fetchPPTStatus]);

  useEffect(() => {
    async function fetchInitialContent() {
      if (backend_type === "ppt") {
        await fetchPPTStatus();
        setLoading(false);
        return;
      }
      try {
  ...
  function renderPPTContent() {
    if (pptStatus === "not_generated") {
      return (
        <div className="flex flex-col items-center justify-center p-12 bg-slate-50 border border-slate-200 rounded-3xl gap-4">
          <div className="w-16 h-16 bg-white shadow-sm border border-slate-100 rounded-2xl flex items-center justify-center text-3xl">📭</div>
          <div className="text-center">
            <h3 className="text-lg font-bold text-slate-800">PPT Not Generated</h3>
            <p className="text-slate-500 max-w-[300px] mt-1">This chapter's PPT has not been generated by the administrator yet.</p>
          </div>
        </div>
      );
    }
    ...
    if (pptStatus === "generating") {
      return (
        <div className="flex flex-col items-center justify-center p-12 bg-indigo-50/30 border border-indigo-100 rounded-3xl gap-6">
          <div className="w-16 h-16 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin" />
          <div className="text-center">
            <h3 className="text-lg font-bold text-slate-800">Generating Presentation...</h3>
            <p className="text-slate-500 mt-1">AI is building your slides. This usually takes 30-60 seconds.</p>
          </div>
        </div>
      );
    }
    ...
    if (pptStatus === "failed") {
      return (
        <div className="flex flex-col items-center justify-center p-12 bg-red-50 border border-red-100 rounded-3xl gap-4">
          <div className="w-16 h-16 bg-white shadow-sm border border-red-200 rounded-2xl flex items-center justify-center text-3xl">⚠️</div>
          <div className="text-center">
            <h3 className="text-lg font-bold text-red-700">Generation Failed</h3>
            <p className="text-red-500 max-w-[300px] mt-1">Something went wrong while generating the PPT. Please contact support.</p>
          </div>
        </div>
      );
    }

    if (pptStatus === "ready" && pptUrl) {
      const encodedUrl = encodeURIComponent(pptUrl);
      const viewerUrl = `https://view.officeapps.live.com/op/embed.aspx?src=${encodedUrl}`;

      return (
        <div className="space-y-8 animate-in fade-in duration-500">
          <div className="bg-emerald-50 border border-emerald-100 rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 shadow-sm">
            <div className="w-16 h-16 md:w-20 md:h-20 bg-white shadow-sm border border-emerald-200 rounded-2xl flex items-center justify-center text-3xl md:text-4xl shrink-0">📊</div>
            <div className="flex-1 text-center md:text-left">
              <h3 className="text-xl font-bold text-emerald-900">Presentation Ready</h3>
              <p className="text-emerald-700 mt-1 text-sm md:text-base">The AI has generated a professional presentation for this chapter.</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              <a 
                ...
                className="flex-1 sm:flex-none px-6 py-3 bg-white border border-emerald-200 text-emerald-700 font-bold rounded-xl hover:bg-emerald-100 transition-all text-center no-underline text-sm"
              >
                📥 Download
              </a>
              <a 
                href={pptUrl} 
                className="flex-1 sm:flex-none px-6 py-3 bg-emerald-600 text-white font-black rounded-xl hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100 active:scale-[0.98] text-center no-underline text-sm"
                target="_blank"
                rel="noopener noreferrer"
              >
                📺 Full Screen
          ...
          <div className="bg-white border-2 border-slate-100 rounded-[2rem] overflow-hidden shadow-2xl relative aspect-video">
            <iframe 
              src={viewerUrl} 
              width="100%" 
              height="100%" 
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-3xl p-8">
            <div className="flex items-center gap-3 mb-6">
               <div className="w-8 h-8 rounded-lg bg-slate-200 flex items-center justify-center text-slate-600 text-sm">ℹ️</div>
               <h4 className="text-[14px] font-black uppercase tracking-widest text-slate-500">Presentation Details</h4>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div className="text-[11px] font-bold text-slate-400 uppercase mb-1">File Format</div>
                <div className="font-bold text-slate-700">PowerPoint (.pptx)</div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div className="text-[11px] font-bold text-slate-400 uppercase mb-1">Interactive</div>
                <div className="font-bold text-slate-700">Animations Enabled</div>
              </div>
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
                <div className="text-[11px] font-bold text-slate-400 uppercase mb-1">Previewer</div>
                <div className="font-bold text-slate-700">Microsoft Office Online</div>
              </div>
            </div>
          </div>
        </div>
