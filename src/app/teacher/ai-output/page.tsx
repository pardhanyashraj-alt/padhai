    ...
    async function fetchInitialContent() {
      try {
        ...
        if (initialMode === "edit") {
            ...
            // Fresh generation from global books
            res = await apiFetch(`/teacher/get-content?book_name=${encodeURIComponent(book)}&class_grade=${grade}&subject=${encodeURIComponent(subject)}&chapter_number=${chapter_number}&content_type=${backend_type}`);
        }

        if (res.ok) {
          const data = await res.json();
          const rawContent = data.content || data[backend_type] || "";
           ...
           <div className="flex gap-4">
              ...
              <button 
                ...
                className={`px-10 py-3 font-black rounded-2xl transition-all shadow-xl active:scale-[0.98] ${
                  published ? "bg-emerald-500 text-white shadow-emerald-100 cursor-default" : "bg-indigo-600 text-white shadow-indigo-100 hover:bg-indigo-700 disabled:opacity-50"
                }`}
              >
